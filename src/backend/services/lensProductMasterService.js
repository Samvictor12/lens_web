...
class LensProductMasterService {
        ...
        try {
            ...
            if (productData.prices && productData.prices.length > 0) {
                ...
                if (coatings.length !== coatingIds.length) {
                    throw new APIError('One or more coating IDs are invalid', 400, 'INVALID_COATING_IDS');
                }
                ...
                if (uniqueCoatings.size !== coatingIds.length) {
                    throw new APIError('Duplicate coating IDs found in prices', 400, 'DUPLICATE_COATING_IDS');
                }
            }
            ...
            const product = await prisma.lensProductMaster.create({
                data: {
                    ...
                    lensPriceMasters: {
                        create: productData.prices?.map(price => ({
    ...
    async addOrUpdateLensPrice(lensId, coatingId, price, userId) {
        try {
            ...
            throw new APIError('Failed to add or update lens price', 500, 'ADD_UPDATE_PRICE_ERROR');
        }
    }
    ...
    async bulkAddOrUpdateLensPrices(lensId, prices, userId) {
        try {
            ...
            throw new APIError('Failed to bulk update lens prices', 500, 'BULK_UPDATE_PRICE_ERROR');
        }
    }
    ...
    async deleteLensPrice(lensId, coatingId, userId) {
        try {
            ...
            if (!existingPrice) {
                throw new APIError('Price not found for this lens-coating combination', 404, 'PRICE_NOT_FOUND');
            }
...
export const createLensProduct = serviceInstance.createLensProduct.bind(serviceInstance);
export const getLensProducts = serviceInstance.getLensProducts.bind(serviceInstance);
export const getLensProductById = serviceInstance.getLensProductById.bind(serviceInstance);
export const updateLensProduct = serviceInstance.updateLensProduct.bind(serviceInstance);
export const deleteLensProduct = serviceInstance.deleteLensProduct.bind(serviceInstance);
export const getProductDropdown = serviceInstance.getProductDropdown.bind(serviceInstance);
export const getProductStats = serviceInstance.getProductStats.bind(serviceInstance);
export const addOrUpdateLensPrice = serviceInstance.addOrUpdateLensPrice.bind(serviceInstance);
export const bulkAddOrUpdateLensPrices = serviceInstance.bulkAddOrUpdateLensPrices.bind(serviceInstance);
export const deleteLensPrice = serviceInstance.deleteLensPrice.bind(serviceInstance);
export const getLensPricesByLensId = serviceInstance.getLensPricesByLensId.bind(serviceInstance);
