  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: err.message || "Could not approve order.",
        variant: "destructive",
      });
    } finally {
      setIsUpdating(false);
        ...
        {(order.urgentOrder || order.freeLens || order.freeFitting) && (
          <SectionCard title="Flags">
            <div className="flex flex-wrap gap-2 py-3">
              {order.urgentOrder && (
                <Badge className="bg-red-100 text-red-700 border-red-200 border">Urgent Order</Badge>
              )}
              {order.freeLens && (
                <Badge className="bg-blue-100 text-blue-700 border-blue-200 border">Free Lens</Badge>
              )}
              {order.freeFitting && (
                <Badge className="bg-green-100 text-green-700 border-green-200 border">Free Fitting</Badge>
              )}
            </div>
          </SectionCard>
        )}
