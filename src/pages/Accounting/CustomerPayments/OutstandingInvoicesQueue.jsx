...
export default function OutstandingInvoicesQueue({
  ...
  const flatTotalOutstanding = flatInvoices.reduce((s, i) => s + (parseFloat(i.outstanding) || 0), 0);

  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between px-1">
        <span className="text-xs text-muted-foreground">
          {flatInvoices.length} pending invoice{flatInvoices.length !== 1 ? "s" : ""}
        </span>
        <Badge className="text-[10px] h-5 bg-orange-100 text-orange-800 hover:bg-orange-100 border-orange-200 font-semibold">
          Total Outstanding: {fmt(flatTotalOutstanding)}
        </Badge>
      </div>
      <div className="rounded-lg border bg-card overflow-hidden">
        <InvoicesTable
