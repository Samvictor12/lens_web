...
import { getBusinessCategories, deleteBusinessCategory } from "@/services/businessCategory";
...
export default function BusinessCategories() {
  ...
  // Filter states
  const [filters, setFilters] = useState(businessCategoryFilters);
  const [tempFilters, setTempFilters] = useState(businessCategoryFilters);
  ...
  const columns = useBusinessCategoryColumns(navigate, handleDeleteClick);
  ...
  const fetchCategories = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to fetch business categories",
        variant: "destructive",
      });
      setCategories([]);
      setTotalCount(0);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setIsDeleting(true);
      await deleteBusinessCategory(categoryToDelete.id);
      
      toast({
        title: "Success",
        description: `Business category "${categoryToDelete.name}" has been deleted successfully.`,
      });
  ...
  const handleClearFilters = () => {
    const clearedFilters = businessCategoryFilters;
    ...
    <div className="flex flex-col h-full p-1 sm:p-1 md:p-3 gap-2 sm:gap-2">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Manage business category master data
          </p>
        </div>
        <div className="flex gap-1.5">
          <Button
            ...
            onClick={() => navigate("/masters/business-categories/add")}
          >
      ...
      <div className="flex-1 min-h-0">
        <Table
          ...
          sorting={sorting}
          pagination={true}
          emptyMessage="No business categories found"
        />
      </div>
