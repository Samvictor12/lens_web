...
import { locationTrayFilters } from "./LocationTray.constants";
import { useLocationTrayColumns } from "./useLocationTrayColumns";
import LocationTrayFilter from "./LocationTrayFilter";
import { Refresh } from "@/components/ui/Refresh";

export default function LocationTrayMain() {
  ...
  const [filters, setFilters] = useState(locationTrayFilters);
  const [tempFilters, setTempFilters] = useState(locationTrayFilters);
  ...
  const columns = useLocationTrayColumns(navigate, handleDeleteClick);
  ...
  const fetchTrays = async () => {
    try {
      ...
      const response = await getLocationTrays(
        pageIndex + 1,
        pageSize,
        searchQuery,
        filters,
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setIsDeleting(true);
      await deleteLocationTray(trayToDelete.id);
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to delete tray.",
        variant: "destructive",
      });
    } finally {
      setIsDeleting(false);
  ...
  const handleClearFilters = () => {
    const clearedFilters = locationTrayFilters;
  ...
  return (
    <div className="flex flex-col h-full min-h-0 overflow-hidden p-1 sm:p-1 md:p-3 gap-2 sm:gap-2">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-lg sm:text-xl md:text-2xl font-bold">Trays</h1>
          <p className="text-xs text-muted-foreground mt-0.5">
            Manage location tray master data
          </p>
        </div>
        <div className="flex gap-1.5">
          <Button
            ...
            onClick={() => navigate("/masters/location-tray/add")}
          >
