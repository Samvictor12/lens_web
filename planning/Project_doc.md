...
* **Sales & Logistics:** Sale order (SO) forms, SO workflows, invoicing/billing (**M.V.V-style Tax Invoice** preview/print with SO customer Ref No and due date from Credit Days; goods description **without DIA**; empty money fields show **0.00** not `—`), and dispatch/delivery agent routing (lists sorted by **createdAt desc**).
* **Inventory Management:** Physical stock items, tray allocation, FIFO stock-picking (STOCK-type PO items are general stock; RX-type PO items are reserved to their SO; SPH/CYL/ADD null/empty ≡ 0 at match time; SO Request Queue + Inventory Dashboard Reserved use in-memory FIFO soft allocation `softReservedQty` + shortage-aware Raise PO; Rx re-raise after QC reject allows a new PO when prior PO is fully RECEIVED), stock-type Inward Queue filtering, tray-to-tray transfers (including same-location / partial qty), inventory transactions, stock-level alerting, and Stock Summary rows keyed by lens + coating + normalized SPH/CYL/ADD (+ location/tray on list) with compact power text after the product name. PO Receive shows Coating and Axis. **Per-eye QC rejection (2026-07-26):** Pre/Post QC can reject Left, Right, or Both; accepted eyes stay reserved on the SO until Pass; reusable rejects → Inward Queue (filtered by SO `procurementType`); scrap at QC writes off immediately (no queue row); Reuse requires location+tray and sets persistent `isReused`; reprocess Issue/Raise PO only for missing eyes. **Reuse power bucketing (2026-07-27):** Reuse canonicalizes the unit to a single-eye SPH/CYL/ADD identity (QcReturn `eyeSide` / `issuedEye`); auto-inward-on-issue stamps only the issued eye; Stock Summary coalesce is eye-flag-aware so qty lands on the returned eye’s power, not the other eye. **Partial reserve SO-link (2026-07-27):** Issue from a multi-qty AVAILABLE row creates SO-linked `RESERVED` child unit(s) with `issuedEye` (source stays AVAILABLE with decremented qty); one unit per eye on Issue; one-eye reject retains the other eye.
* **Quality & Shop Floor:** Check sheets, production operators, quality operator screens, and barcode/QR wedge scanning interfaces.
* **Financial Accounting:** Ledger, **Account Groups & industry Chart of Accounts**, **Customer & Vendor Payment Vouchers** (invoice-first vendor pay; cancel→reverse; Outstanding List UI; cash/bank account selects show `currentBalance`), **Vendor Invoices** (eligible-PO picker), Customer Credit Note / Vendor Debit Note (document-only), Bank Reconciliation, Expenses, **Income** (Bank Transfer, Loan, …; From/To show `currentBalance`; Category reload on dialog open) + **Bank Account manage**, Group Summary / Balance Sheet / Profit & Loss reporting.
