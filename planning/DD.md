...
* **Tray-to-tray TRANSFER:** Same-location transfers are allowed when source and destination trays differ. Full transfer relocates the existing `InventoryItem`; partial transfer decrements source quantity and creates a new item at the destination. Both source and destination `InventoryStock` buckets update inside the same Prisma transaction client. **Unit-cost ledger (req-001):** qty N → N `TRANSFER` rows copying source `unitPrice`; each tags one OPEN inward/transfer via `parentTransactionId`; source `remainingQty` 0 → CONSUMED. New transfer rows are OPEN sources (`remainingQty=1`). Operator Adjustment type removed; `POST /transactions` rejects `ADJUSTMENT`. QC reuse writes `INWARD_DIRECT` (not Adjustment).
* **Inward unit price (req-001):** Direct inward uses operator `costPrice` immediately. PO inward/`INWARD_PO` stays unpriced until vendor bill: `unitPrice = VendorInvoiceItem.subtotalAmount / sum(INWARD_PO.quantity for that PO)`. Helper: `src/backend/services/inventoryUnitCostLedger.js`.
...
* **Inventory Audit tab (req-013, 2026-08-31; PRD-4.14, 2026-09-14):** `/inventory/{stock|rx}/audit` — Tray Transfer (`POST /transactions` type=`TRANSFER`, same godown, dest tray ≠ source), Cycle Count (`/api/inventory/cycle-counts*`), Manual Add Stock (`INWARD_DIRECT` single spec), Initialize Stock (bulk grid via `POST /items/bulk-grid` alias), Spec Threshold editor. UI: `InventoryAuditTab.jsx`, `InventoryTrayTransferForm.jsx`, `InventoryCycleCountSection.jsx`. Services: `inventoryCycleCount.service.js`, `inventoryCycleCount.helpers.js`. Godown-scoped dropdowns identical to other inventory tabs. Initialize Stock removed from Dashboard.
* **Spec-level stock thresholds (req-013):** `InventorySpecThreshold` rows keyed by `lens_id + godownType + sph/cyl/add` (normalized). `specQty` = sum `InventoryItem.quantity` grouped by lens + coalescePower within `inventoryItemGodownWhere`. Alerts computed at read time (no `InventoryAlert` writes): OUT (threshold exists, qty 0), LOW (qty > 0 and < min), OVER (max set and qty > max). High/Low/Out pie on the dashboard uses the same engine as `GET /spec-alerts` (High = OVER). Product-level `LensProductMaster.minThresholdQty`/`maxThresholdQty` no longer drive dashboard KPIs.
* **Inventory Dashboard (req-001 / PRD-4.13 / PRD-4.14):** Five KPI cards — Total Products, Total Stock units, Total Value, this-month qty as `inward / outward`, this-month value as `₹in / ₹out` — plus a cycle-count card from `GET /dashboard` `cycleCount` (`completionPct`, `accuracyPct`, matched / variance / pendingRecount, traysCounted / traysInScope). Click → `/inventory/{slug}/audit#cycle-count`. High/Low/Out pie + % bars (click → spec popup). Low/Out Raise PO hydrates `/masters/purchase-orders/add` Bulk `lensBulkSelection` with qty=`minQty` (single `lens_id`). Trend: `GET /reports/value` with 7/15/30/60/90 day windows. Top/Low 10: spec-grain `OUTWARD_SALE` via `GET /reports/top-low-selling?days=30|60|90`. Queue: donut + progress bars (`pendingInwardsCount` vs `soQueueCount`); click navigates to `/inventory/{stock|rx}/inward` or `.../request-queue` (no FIFO popup). Bottom row 60/40 Stock Summary lists only Products, Locations, Trays, Stock units, Total value (`locationCount`/`trayCount` distinct non-null ids). UI: `src/pages/Inventory/InventoryDashboard.jsx`. Display binds `specQty`/`totalStockUnits` — never `currentQty`. **Total Value** uses the same grouping cost×qty helper as Stock Summary (`aggregateStockSummaryTotals`); do not sum `InventoryStock.totalStock * avgCostPrice` (bucket table is often empty vs items). Finance Dashboard `position.inventoryValue` reuses this `totalValue` from `getInventoryDashboardEnhanced({})` (company-wide, RX-source exclusion on items).
...
### C. Financial Ledgers
* **Chart of Accounts (COA):** Three-level Tally-style hierarchy — **Primary Group → Account Group → Posting Ledger**. `AccountGroup` classifies ledgers for Balance Sheet sections and P&L (Direct/Indirect income/expense via `pnlClassification`). **req-003:** Liability parents are Capital (`GRP-CAPITAL`), Loans (`GRP-LOANS` + `AC-2004`), Current Liabilities. Capital ledgers are LIABILITY (not EQUITY). Balance Sheet roots: `GRP-ASSETS` + `GRP-LIABILITIES` only.
...
* **Invoice due date:** `Invoice.dueDate` = `billDate` + `Customer.credit_days` when client omits override. **req-002:** `Invoice.billDate` is the customer bill date (editable, default today) and the sale FT `transactionDate`. Vendor bill FT date = `VendorInvoice.invoiceDate`; payment FTs use voucher `paymentDate`.
* **Sale inventory credit (req-002):** On invoice issue, if sum of linked `OUTWARD_SALE.unitPrice` ≥ 0.01: Dr AC-4001 (COGS) / Cr AC-1004 (Inventory). Skip both lines when cost is under 0.01. Existing Dr Customer / Cr Sales / Cr GST unchanged. `reverseInvoice` reverses COGS+Inventory when those lines were posted.
...
* **Vendor Invoice create:** Eligible PO list excludes POs already linked to a non-cancelled `VendorInvoice`. **GL accrual (req-014):** `postVendorInvoice` on bill registration — Dr AC-1004 (+ AC-1005 if tax), Cr vendor AP; `referenceType: VENDOR_INVOICE`. PO receipt is ops-only (no `postPurchaseReceipt`). Cancel/update reverse via `postReversingTransaction` when unpaid. **req-001:** bill create/update also syncs `INWARD_PO` unit prices (ops ledger; GL dates/COGS remain req-002).
* **Expenses:** Category from Expense Category (type auto-fills); optional `Expense.dueDate`; Payment Account from `getCashBankLedgers()` (service returns array — do not check `.success` on client). **req-003:** mark/cash reject `expenseDate` after `dueDate`; GL date = `expenseDate` or pay `paymentDate`.
* **Income & Bank Accounts (2026-07-25):** Income vouchers use **From + To** ledgers; posting **Dr To / Cr From**. **req-003:** Income From = `GET /api/ledgers/capital-posting`; Loan From = `GET /api/ledgers/loans-posting`; To = `GET /api/ledgers/cash-bank`. FT date = `incomeDate`. Bank Account manage CRUD for GRP-CASH/GRP-BANK. Permissions `income`, `income_categories`, `bank_accounts` must stay in `role.constants.js` + `role-seed.js` (KB-026/034).
