...
* **RX-sourced stock exclusion (Pass J):** "RX" is defined at the `PurchaseOrder` level, not on the product/master data — an `InventoryItem` is RX-sourced iff its `purchaseOrder.saleOrderId` is non-null. The correct Prisma filter is `NOT: { purchaseOrder: { saleOrderId: { not: null } } }` (module-level `RX_SOURCE_EXCLUSION_FILTER` in `inventory.service.js`) — **not** the simpler `{ purchaseOrder: { saleOrderId: null } }` form used by the Inward Queue filter, since on an optional to-one relation that form also drops every row with `purchaseOrderId = null` (i.e. all manually-initialized stock). When adding RX-exclusion to any new query, reuse the `NOT` form.
