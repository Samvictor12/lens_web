...
* **Stock Summary grouping (2026-07-26):** List aggregation (`getInventoryStockWithGrouping`) keys by `lens_id|coating_id|location_id|tray_id|normalizedSph|normalizedCyl|normalizedAdd` (no `Type_id`/`category_id` splitters). Pivot (`getInventoryStockPivot`) keys by `lens_id|coating_id|normalizedSph|normalizedCyl|normalizedAdd` (no `lensType.id`). Power normalize (`normalizePowerValue`) treats null/empty as `0` and uses consistent `toFixed(2)` so `"0"` ≡ `"0.00"`. Rows still return flat `sph`/`cyl`/`add` and compact power via `formatItemPowerRange`. List SPH/CYL/ADD filters use pivot-equivalent OR matching. `InventoryStock` bucket schema remains product/location-level (not power-level).
